...
FEATURES_TO_USE ='mfcc'# 'melspectrogram'  # {'mfcc' , 'logfbank','fbank','spectrogram','melspectrogram'}
featuresExist = True
...
def train(SEED, area_width, area_height,AUG=True):
    setup_seed(SEED)
    MODEL_NAME = 'CNN_mfcc_seed{}'.format(SEED)#area_width, area_height, SEED)
    data_dir = '../IEMOCAP/'
    spec_dir='spec/'
    train_files = []
    train_files2 = []
    ...
    with open(data_dir + '/IEMOCAP_dev_{}.csv'.format(SEED)) as f:
        ...
        for line in fr:
            ...
            if(AUG):
                for i in range(3):
                    train_files.append(data_dir + '/' + line.split('\t')[2]+'.'+str(i+1))
                    train_files.append(data_dir + '/' + line.split('\t')[2]+'.'+str(i+5))
    ...
    #   	    fr = f.readlines()
    #        for line in tqdm(fr):
    #            spec_path=glob.glob(spec_dir+'/spec_'+line.split('\t')[2]+'_{}_*'.format(i))
    #            for j in spec_path:
    #                spec=np.load(j,allow_pickle=True)
    #                spec=tf.expand_dims(tf.expand_dims(spec,0),-1)
    #                spec = tf.cast(spec, tf.float32)
    #                train_X_features=tf.concat([train_X_features,spec],0)
    #                y=str(os.path.basename(j).split('-')[2])
    #                y=LABEL[LABEL_DICT1[y]]
    #                y=tf.expand_dims(y,0)
    #                train_y=tf.concat([train_y,y],0)
    #    with open(data_dir + '/IEMOCAP_dev_{}.csv'.format(SEED)) as f:
    #        fr = f.readlines()
    #        for line in fr:
    #            spec_path=glob.glob(spec_dir+'/spec_'+line.split('\t')[2]+'_{}_*'.format(i))
    ...
    model = MODEL.AACNN(area_width, area_height)
    ...
    np.save('./test/CNN_mfcc_SERtest_{}.npy'.format(SEED), result)
...
if __name__ == '__main__':
    ...
    log_name = 'CNN_mfcc.log1'#.format(area_width, area_height)
    logfile = log_name
    ...
    for seed in SEED:
        train(seed, area_width, area_height,False)
